...
O modelo final (HGB de classificação) é aplicado às 5.452 linhas de 2024 para estimar o risco de não atingir a meta de 2025 (`n_aplicacao_2024`), e todas elas aparecem no ranking de risco. Desses 5.452, 100 municípios não têm meta pactuada para 2025 (`n_sem_meta_2025`); para eles `gap_previsto_2025` e `acima_da_margem` ficam NaN/False (não há meta para comparar), mas ainda recebem `prob_nao_atingir_2025` do classificador e continuam no ranking.
...
- **Um em cada quatro municípios avaliados está fora da margem de erro da meta de 2025.** Das 5.452 aplicações do Modelo B em 2024, 1.267 municípios (23,2%) têm probabilidade prevista de não atingir a meta de 2025 além da margem de `ic95` (`acima_da_margem`); considerando só os 5.352 municípios que de fato têm meta pactuada para 2025 (excluindo os 100 sem meta, para os quais `acima_da_margem` não é computável e vem `False` por padrão), a proporção é 1.267/5.352 ≈ 23,7%. O risco é concentrado: os 20 municípios de maior `prioridade` (risco × volume) somam 118.475 crianças previstas como não alfabetizadas, 15,7% do total nacional de 752.951 estimado pelo ranking.
...
- **Cobertura de `had_ai` cai de 2023 para 2024, não corrigido nesta entrega.** Em `data/processed/base_modelagem_municipio.parquet`, a feature `had_ai` (uma das 70 do Modelo B) está 100% preenchida nas linhas de treino de 2023, mas só 73,4% preenchida nas linhas de aplicação de 2024 (26,6% de nulos) — cerca de 26,6% das linhas de 2024 recebem valor imputado pela mediana para uma coluna que o modelo viu sempre preenchida no treino. É o mesmo tipo de deslocamento treino→aplicação que já levou a excluir `pct_familias_bolsa_familia` e as colunas `pct_va_*` do Modelo B, mas de magnitude menor — uma feature entre 70, com imputação por mediana.
