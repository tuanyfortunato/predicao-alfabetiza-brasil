...
**Aplicação em produção:** o HGB de classificação é aplicado às `n_aplicacao_2024` = 5.452 linhas de 2024 para estimar o risco de não atingir a meta de 2025, e todas elas aparecem no ranking (`reports/ranking_risco_municipios.csv`). Dessas 5.452, `n_sem_meta_2025` = 100 municípios não têm meta pactuada para 2025; para eles `gap_previsto_2025` e `acima_da_margem` ficam NaN/`False` (não há meta para comparar), mas ainda recebem `prob_nao_atingir_2025` do classificador e não são excluídos do ranking. `n_acima_da_margem` = 1.267 municípios (23,2% das 5.452 aplicações, ou 23,7% se a base for só os 5.352 municípios com meta pactuada) têm probabilidade prevista de não atingir a meta além da margem de `ic95` — não só levemente abaixo por ruído estatístico. `prob_media_nao_atingir_2025` = 0,2075 (probabilidade média entre as 5.452 aplicações).
...
### 6.5 Disclosure de qualidade de dados — cobertura de `had_ai` cai de 2023 para 2024

**Achado, registrado sem esconder:** em `data/processed/base_modelagem_municipio.parquet`, a feature `had_ai` (uma das 70 do Modelo B) está **100,0% preenchida** nas linhas de treino de 2023, mas apenas **73,4% preenchida** nas linhas de aplicação de 2024 (26,6% de nulos). É o mesmo tipo de deslocamento treino→aplicação que já levou a excluir `pct_familias_bolsa_familia` e as quatro colunas `pct_va_*` do Modelo B (`NAO_FEATURES_B`, seção 5.2 acima) — mas `had_ai` passou despercebido dessa exclusão porque seu deslocamento é menor.

**Efeito prático:** cerca de 26,6% das linhas de aplicação de 2024 recebem um valor imputado pela mediana para `had_ai` que o modelo não viu como imputado durante o treino (onde a cobertura era total). O efeito é limitado — é uma feature entre 70, com imputação por mediana, não pela mediana condicional em nada mais grave — mas não foi corrigido nesta entrega, de forma análoga (porém menor em magnitude) ao que já foi feito para `pct_familias_bolsa_familia`/`pct_va_*`. Fica registrado aqui e no README (seção "Limitações do projeto") como limitação conhecida.
